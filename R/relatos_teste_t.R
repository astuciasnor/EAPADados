...
#
# Arquitetura (fonte canonica unica):
#   calcular_*()  -> executa o teste e devolve UMA lista com as estatisticas.
#                    E a definicao canonica de cada analise.
#   mostrar_*()   -> apenas FORMATA a lista canonica como tabela (tibble).
#   relatar_*()   -> apenas FORMATA a lista canonica como frase em portugues.
# Assim, tabela e narrativa nunca divergem: ambas leem o mesmo calculo.
# A CatalyseR (IDE) e o livro EAPA consomem estas mesmas funcoes.
#
# Observacao: as strings de SAIDA (cabecalhos de tabela e narrativa) usam
# acentuacao correta em portugues (arquivo em UTF-8) porque vao para o
# relatorio final. Os comentarios sao mantidos em ASCII.
# =============================================================================

# ---- Utilitario: formato numerico brasileiro (virgula decimal) --------------
# Funcao interna (nao exportada).
fmt <- function(x, dig = 2) {
  if (is.null(x) || length(x) == 0 || is.na(x)) return("-")
  formatC(x, format = "f", digits = dig, decimal.mark = ",")
}

# ---- Interpretacao do tamanho de efeito (d de Cohen, em modulo) -------------
# Funcao interna (nao exportada).
...
# ---- Rotulo dinamico do intervalo de confianca ------------------------------
# Funcao interna (nao exportada).
...
#' estatisticas (media, t, gl, p, IC, d de Cohen, efeito, significancia).
#' E a fonte unica consumida por \code{mostrar_teste_t} e \code{relatar_teste_t}.
#'
...
#' Relato textual em portugues do teste t para uma amostra
#'
#' @param x vetor numerico.
#' @param mu valor de referencia (media hipotetica sob H0).
#' @param nome rotulo da variavel.
#' @param conf nivel de confianca do IC (padrao 0,95).
#' @return Uma string com marcacao Markdown para italico no docx.
#' @export
...
#'
#' @param formula_obj formula y ~ x (ex.: peso ~ grupo).
#' @param data data.frame com as variaveis.
#' @param equal_var logico; FALSE usa Welch (variancias desiguais).
#' @param conf nivel de confianca do IC (padrao 0,95).
#' @return Uma lista com as estatisticas do teste.
#' @export
calcular_teste_t_ind <- function(formula_obj, data, equal_var = FALSE, conf = 0.95) {
  ...
  # Cohen's d com desvio padrao combinado (pooled)
...
#' Tabela de resultados do teste t para duas amostras independentes
#'
...
#' Relato textual em portugues do teste t para duas amostras independentes
#'
#' @inheritParams calcular_teste_t_ind
...
#'
#' @param x1 vetor numerico (momento 1 / antes).
#' @param x2 vetor numerico (momento 2 / depois).
#' @param conf nivel de confianca do IC (padrao 0,95).
#' @return Uma lista com as estatisticas do teste.
#' @export
calcular_teste_t_pareado <- function(x1, x2, conf = 0.95) {
  ...
  sd_dif <- stats::sd(dif, na.rm = TRUE)
  d      <- m_dif / sd_dif
  list(
...
#' Tabela de resultados do teste t pareado
#'
#' @param x1 vetor numerico (momento 1 / antes).
#' @param x2 vetor numerico (momento 2 / depois).
...
#' Relato textual em portugues do teste t pareado
#'
#' @param x1 vetor numerico (momento 1 / antes).
#' @param x2 vetor numerico (momento 2 / depois).
#' @param nome1 rotulo do momento 1.
...
# =============================================================================
# FORMATACAO DA TABELA (identidade Ocean Gradient, saida docx)
# =============================================================================
...
flextable_ocean <- function(tab) {
  if (!requireNamespace("flextable", quietly = TRUE)) {
    stop("O pacote 'flextable' e necessario para formatar a tabela. ",
         "Instale com install.packages('flextable').", call. = FALSE)
  }
