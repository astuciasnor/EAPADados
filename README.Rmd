...
ggplot(resumo_art, aes(racao, media_taxa, fill = racao)) +
  geom_col(color = "white", width = 0.6) +
  geom_errorbar(aes(ymin = media_taxa - se, ymax = media_taxa + se),
                width = 0.2) +
  stat_compare_means(
    comparisons = list(c("A", "B")),   # define qual par comparar
    method      = "t.test",
    label       = "p.signif",          # asteriscos
    hide.ns     = FALSE,               # mostra “ns” se não houver significância
    label.y     = max(resumo_art$media_taxa + resumo_art$se) * 1.1
  ) +
  labs(
